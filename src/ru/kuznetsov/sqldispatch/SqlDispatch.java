...
//			else throw new SQLException("Unknown fild name\"" + currentWord + "\"");
    ...
    private boolean dataSource() throws SQLException {
        ...
        DruidDatasource queryDatasource = null;
        for (DruidDatasource currentDatasource : datasources) {
            if (currentDatasource.getDatasouceName().equals(dataSource))
                queryDatasource = currentDatasource;
        }
        
        if (queryDatasource == null) {
            throw new SQLException("Unknown datasource: \"" + dataSource + "\"");
        }
        
        for (String field : queryFields) {
            if (queryDatasource.getDimensions().contains(field)) {
                queryDimensions.add(field);
            } else if (queryDatasource.getMetrics().contains(field)) {
                queryMetrics.add(field);
            } else {
                throw new SQLException("Unknown fild name\"" + field + "\"");
            }
        }
    ...
    public String toGson() {
            ...
            query.addProperty("granularity", "all");
            ...
            intervals.add("2018-01-01T00:00:00.000/" + sdf.format(new Date()));
